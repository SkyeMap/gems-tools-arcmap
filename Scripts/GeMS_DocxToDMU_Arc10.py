...
def parseUnitPara(para):
    ...
    for fmt in ('<b>','</b>'):
        ...
        age = age.replace(fmt,'')
    if debug: addMsgAndPrint(label+'**'+name+'**'+age)
    if debug: addMsgAndPrint('    '+desc[0:20])
...
def rankParaStylesAreEqual(style1,style2):
    if debug: addMsgAndPrint(style1+'  '+style2)
...
for paragraph in paragraphList:  # each paragraph is a list: [style, text]
    ...
    if paraStyle[0:3] == 'DMU' and paraStyle <> 'DMU-Heading1':
        ...
        if styleType(paraStyle) == 'DMUParagraph':
            ...
            if debug: addMsgAndPrint('3')
        else:
            if debug: addMsgAndPrint('2b')
            label = ''; name = ''; age = ''; description = ''
...
for n in range(1,len(msRows)):
    ...
    if rankParaStylesAreEqual(hKeyStyleDict[str(lastHKey)],row[4]): # is sibling to lastHKey
